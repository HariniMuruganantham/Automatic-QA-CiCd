...
and uses OpenAI to produce an intelligent failure analysis.
"""
...
def load_all(results_dir: str) -> dict:
    ...
    def auto_parse(path):
        """Try jest format first, fall back to pytest."""
        if not path:
            return _empty("file not found")
        try:
            raw = json.loads(Path(path).read_text())
            if "numTotalTests" in raw:
                return parse_jest(path)
            return parse_pytest(path)
        except Exception as e:
            return _empty(str(e))
...
def ai_analysis(results: dict, project: str,
    ...
    for suite, data in results.items():
        ...
        if "p95_ms" in data:
            lines.append(
                f"{suite}: p95={data['p95_ms']}ms, "
                f"error_rate={data['error_rate']}%, "
            ...
            lines.append(
                ...
                + (f" | note: {data.get('note','')}" if data.get("note") else "")
            )

    prompt = f"""QA test run summary for project '{project}':
{chr(10).join(lines)}

Write a concise 3-4 sentence plain-text analysis:
- Overall health and pass rate
- Most critical failures (be specific about test/suite names)
...
.ob {{
  font-size:18px; font-weight:700; padding:10px 24px;
  border-radius:10px; border:1.5px solid {oc}44;
  background:{oc}18; color:{oc}; white-space:nowrap;
}}
.div {{
  height:1px;
  background:linear-gradient(90deg,transparent,#6366f1,#a78bfa,#34d399,transparent);
  margin:0 0 28px;
}}
...
.sl {{ font-size:10px; color:#6b7280; margin-top:6px;
...
.foot {{
  ...
  margin-top:36px; padding-top:18px; border-top:1px solid #1e2030;
}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <div class="hdr">
    <div>
      <div class="title">QA Report &mdash;
        <span class="grad">{project}</span>
      </div>
      <div class="meta">
  ...
  <div class="stats">
    <div class="stat">
      <div class="sv" style="color:#10b981">{total_passed}</div>
      <div class="sl">Passed</div>
    </div>
    <div class="stat">
      <div class="sv" style="color:#ef4444">{total_failed}</div>
      <div class="sl">Failed</div>
    </div>
    <div class="stat">
      <div class="sv" style="color:#6366f1">{load.get("total_requests", 0)}</div>
      <div class="sl">Load Req</div>
    </div>
    <div class="stat">
      <div class="sv" style="color:#f59e0b">
        {load.get("p95_ms", 0)}<span style="font-size:15px">ms</span>
      </div>
      <div class="sl">p95 Latency</div>
    </div>
    <div class="stat">
      <div class="sv"
        style="color:{'#10b981' if load.get('error_rate',100)<5 else '#ef4444'}">
        {load.get("error_rate", 0)}<span style="font-size:15px">%</span>
      </div>
      <div class="sl">Error Rate</div>
    </div>
  </div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Passed / Requests</th>
          <th>Failed / Latency</th>
  ...
  <div class="foot">
    <strong>Automatic-QA-CiCd</strong> &nbsp;&middot;&nbsp;
    OpenAI {model} &nbsp;&middot;&nbsp;
